...
export function MonsterBurstMesh() {
  return (
    <group renderOrder={999}>
      ...
      {/* lampu efek TIDAK dipasang di sini: grup ini di-hide/show tiap
          fase, dan menambah/menghapus light dari scene memaksa SEMUA
          material Three.js dikompilasi ulang (sumber stutter terbesar).
          Lampu dipasang permanen di Angler.tsx, hanya intensitasnya yang
          dianimasikan (0 = mati, hasil visual sama persis). */}
    </group>
  );
}

/** k = 0..1 progres efek (0 = monster menembus permukaan), t = waktu global */
export function animateBurst(
  g: THREE.Group,
  k: number,
  t: number,
  /** lampu permanen (sibling `g`, ruang koordinat sama) */
  ...
  let cached = g.userData["_burstRefs"] as Refs | undefined;
  if (!cached) {
    cached = {
    ...
    g.userData["_burstRefs"] = cached;
  }
  const refs: Refs = cached;
  ...
  if (light) {
    ...
    light.position.set(g.position.x, g.position.y + 8, g.position.z);
  }
}