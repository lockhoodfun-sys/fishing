...
import { GRAPHICS, useGraphics, type GraphicsTier } from "@/hooks/useGraphics";

const damp = (cur: number, to: number, k: number, dt: number) =>
  cur + (to - cur) * (1 - Math.exp(-k * dt));

/** Shared scratch colours so the per-frame lerps allocate nothing. */
...
function CloudCeiling({ flash }: { flash: React.MutableRefObject<number> }) {
  ...
  useFrame((state, raw) => {
    ...
    // reuse one scratch Color instead of allocating a new one every frame
    (u["uColor"]!.value as THREE.Color).lerp(
      _scratchColor.set(target.cloudColor),
      1 - Math.exp(-1.5 * dt),
    );
...
/** Drop budget per graphics tier — High keeps the original look. */
const RAIN_BUDGET: Record<GraphicsTier, number> = {
  low: 1200,
  medium: 2600,
  high: MAX_DROPS,
};
...
function Rain() {
  ...
  const budget = RAIN_BUDGET[tier] ?? MAX_DROPS;
  ...
  // free the GPU buffer when the scene unmounts
  useEffect(() => () => geometry.dispose(), [geometry]);

  useFrame((state, raw) => {
    const dt = Math.min(raw, 0.05);
    amount.current = damp(amount.current, target.rain, 2.2, dt);
    // Only ever simulate the drops actually being drawn, capped by the
    // graphics tier — previously all 5000 were integrated + uploaded every
    // frame regardless of weather or quality setting.
    const active = Math.floor(amount.current * budget);
    ...
    // upload only the slice in use instead of the whole 5000-drop buffer
    const attr = geometry.attributes["position"] as THREE.BufferAttribute;
    attr.updateRanges.length = 0;
    attr.addUpdateRange(0, active * 6);
...
function Atmosphere({ flash }: { flash: React.MutableRefObject<number> }) {
  const kind = useWeather((s) => s.kind);
  const shadowMapSize = useGraphics((s) => GRAPHICS[s.tier].shadowMapSize);
  const target = WEATHER[kind];
  ...
  useFrame((_, raw) => {
    ...
    fogColor.lerp(_scratchColor.set(target.fogColor), lerp);
    sunColor.lerp(_scratchColorB.set(target.sunColor), lerp);
