...
export function playFootstep(
  ...
  // Some browsers suspend the AudioContext between gestures or on backgrounding.
  // Resume it here so footsteps don't go silent after the first interaction.
  const needsResume = ctx.state !== "running";
  if (needsResume) {
    void ctx.resume().catch(() => undefined);
  }

  // If the context was just resumed, schedule sounds a tiny bit in the future
  // so they actually play once the clock starts advancing again.
  const t = ctx.currentTime + (needsResume ? 0.05 : 0);
